@echo off
:: Rebuild/Repack files inside netfx_Full_LDR.mzz
set BuildMzz=1
...
BIN\7z.exe e %ndpack% -o%ndpver%\temp netfx_Full_LDR_x64.msi netfx_Full_LDR_x86.msi netfx_Full_LDR.mzz >nul
if %msp%==0 (
goto :slim
)
...
set name="Microsoft .NET Framework 4.5.2"
set desc="Microsoft .NET Framework 4.5.2; Copyright (C) Microsoft Corporation, All rights reserved."
...
if not %msp%==0 (
cscript //B WiFilVer.vbs netfx_Full_LDR_x86.msi /u
cscript //B WiFilVer.vbs netfx_Full_LDR_x64.msi /u
)
cscript //B WiSumInf.vbs netfx_Full_LDR_x86.msi Subject=%name% Comments=%desc% Revision=%guid86% Words=4
cscript //B WiSumInf.vbs netfx_Full_LDR_x64.msi Subject=%name% Comments=%desc% Revision=%guid64% Words=4

if %BuildMzz%==0 goto :skip
if not exist "%_wix%\heat.exe" goto :skip
echo.
echo Rebuild netfx_Full_LDR.mzz . . .
cscript //B WiMakCab.vbs netfx_Full_LDR_x64.msi netfx
mkdir SourceDir
cd SourceDir
...
ren product.cab netfx_Full_LDR.mzz
rd /s /q ProgramFilesFolder\ Windows\ SourceDir\
cscript //B WiSumInf.vbs netfx_Full_LDR_x86.msi Words=0
cscript //B WiSumInf.vbs netfx_Full_LDR_x64.msi Words=0
